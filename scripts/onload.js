...
`;
  ...
  discordChatType.innerHTML = `
    ...
    position: absolute;
">
    <button id="discordChatType" style="width: 250px; padding: 2px; border: none; border-radius: 10px; background: linear-gradient(to right, #007bff, #0047ab); color: #fff; font-size: 16px; cursor: pointer; transition: opacity 0.3s;${settings.config.addDiscord ? 'display: block;' : 'display: none;'}">Open Discord</button>
    <div>
    `;
  document.body.appendChild(discordChatType);
  document.getElementById("discordChatType").addEventListener("click", () => {
