...
<div ${packages.settings ? "" : 'style="display: none;"'}>
    ...
    <div ${packages.settings_checkboxes ? "" : 'style="display: none;"'}>
        <br />
        <br />
        <div class="name">Minor Changes</div>
        <hr />
        <div>
        ...
        <label for="nameIcon">Name Icon (Deprecated)</label>
        <br />
    ...
    <div ${packages.settings_autoUpgrade ? "" : 'style="display: none;"'}>
        <br />
        <br />
        <div class="name">Auto Upgrading</div>
        <hr />
        <div id="preset"></div>
        <br />
    ...
    <div ${
      packages.settings_superpower_text || packages.settings_html_addons
        ? ""
        : 'style="display: none;"'
    }>
        <br />
        <br />
        <div class="name">Text Customize</div>
        <hr />
    </div>
    ...
    <div id="customSkins" ${
      packages.settings_custom_skins || true ? "" : 'style="display: none;"'
    }>
        <br />
        <br />
        <div class="name">Skin Import</div>
        <hr />
        <svg style="width: 15px; height: 15px;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
        </svg>
    ...
    <button id="deflyMonsterImport" type="button" class="button" lang="en" style="display: none;">Defly Monster Skin Import</button>
  ...
  document.getElementById("noSignInCheckbox").onchange = () => {
    settings.config.noSignInCheckbox = document.getElementById("noSignInCheckbox").checked;
    if (settings.config.noSignInCheckbox) {
      features.noSignInCheckbox.up();
    } else {
      features.noSignInCheckbox.down();
    }
    settings.save();
  };
  ...
  advancedSettings.load(
    `
      ...
      settings.config.licenseKey && settings.config.licenseKey.length !== 0
        ...
        <br />
        <div>
            <button id="resetZoomCal" style="margin: auto;" type="button" class="button" lang="en">Reset Zoom Calibration</button>
        </div>
    `,
    document.body
  );
  ...
  document.getElementById("resetZoomCal").onclick = () => {
    localStorage.removeItem("initPR");
  };
