...
<div ${packages.settings ? "" : 'style="display: none;"'}>
    ...
    function toggleAlt() {
      ...
      document.querySelector("#accounts").style.display =
        document.getElementById("alt").checked ? "block" : "none";
      settings.save();
    }
    ...
    document.getElementById("MiniMapZoom").onchange = () => {
      ...
      settings.config.MiniMapZoom =
        document.getElementById("MiniMapZoom").value;
      settings.save();
    ...
    let skins = new dropDown(
      ...
      (me) => {
        settings.config[me.id] = settings.config[me.id].filter(
          (obj) =>
            obj[me.key] !== document.querySelector(`#${me.id}Select`).value
        );
        me.loadItems();
        settings.config.activeSkinName = "";
        settings.save();
    ...
    document.getElementById("checkForPrem").onclick = () => {
      ...
      new Promise(async (resolve, reject) => {
        ...
        document
          .querySelector("#my-account > div.center > div > button")
          .click();
        // Save to settings
        settings.save();
        send(
        ...
        testURL(
          ...
          async (response) => {
            ...
            if (response.status === 200) {
              ...
              settings.save();
              alert(
                `Your account is now connected! Welcome back ${e.data.name}`
              );
              location.reload();
