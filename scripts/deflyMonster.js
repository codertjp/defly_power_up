...
<div id="tab-custom" style="display: none;" class="customTab">
    ...
    <br />
    <br />
    <button style="padding: 10px;" id="custom-monster-import" type="button" class="button" lang="en">IMPORT</button>
</div>
...
document.getElementById("custom-monster-import").onclick = () => {
  // custom: [
  //     {
  //       name: "Astro.",
  //       skinSets: [],
  ...
  const creator =
    monsterJson.custom[
      parseInt(document.querySelector("#tab-custom-select-creator").value)
    ].name;
  console.log(
    monsterJson.custom,
    parseInt(document.querySelector("#tab-custom-select-creator").value),
  ...
  const set =
    creator.skinSets[
      parseInt(document.querySelector("#tab-custom-select-set").value)
    ];
  const name =
    set.skinSets[
      parseInt(document.getElementById("tab-in-game-select-name").value)
    ].name;
  fetch(`https://skins.defly.monster/skin/${creator}/${set}/${name}`)
    .then((e) => e.json())
    .then((skin) => {
      settings.config.skins.push({
        name: prompt("Enter a Name For This Skin", name),
